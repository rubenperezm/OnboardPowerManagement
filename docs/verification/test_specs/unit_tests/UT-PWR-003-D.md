# Unit Test Specification: UT-PWR-003-D

## 1. Purpose

The purpose of this unit test is to verify that the PowerManager software component does not
return from `SAFE` mode to `NORMAL` the upper safe voltage threshold is not exceeded by 5 %, provided
...
- SAFE mode was entered due to an overvoltage condition
...
2. Force the operational mode to `SAFE` due to an overvoltage condition
3. Query the current operational mode
4. Inject a battery voltage value 29.2V, outside the safe range
5. Query the current operational mode
6. Inject a battery voltage value 27.8V, not exceeding the upper safe voltage threshold by 5 %
...
- Upon injection of the first voltage conditions (29.2V), the PowerManager remains in `SAFE` mode
- Upon injection of the second voltage condition (27.8V), the PowerManager remains in `SAFE` mode
