...
does not flag overvoltage conditions when battery voltage values are within the configured voltage thresholds.
...
3. Query the internal overvoltage detection status
4. Inject a battery voltage value above the configured upper voltage threshold
5. Query the internal overvoltage detection status
6. Inject a battery voltage value of within the configured recovery voltage thresholds
7. Query the internal overvoltage detection status

## 7. Expected Results

- The initial operational mode after initialization is `NORMAL`
- The overvoltage condition is not flagged by the PowerManager logic when voltage is within thresholds
...
- PASS if the PowerManager:
  - Does not flag overvoltage condition when voltage is within thresholds
- FAIL otherwise
