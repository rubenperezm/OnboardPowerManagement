...
PowerManager::PowerManager(const Config& config) : config(config), m_lastMeasurements(std::nullopt), 
    m_lastHealthStatus(std::nullopt), m_overcurrentStartTimestamp(std::nullopt),
    m_isShutdown(false), m_currentMode(PowerMode::NORMAL), m_lastCommand(std::nullopt) {}
...
bool PowerManager::checkOffConditions() const {
    return m_lastHealthStatus.has_value() && m_lastHealthStatus.value().anyFailed;
}
